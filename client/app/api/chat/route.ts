...
const VALIDATION_MODEL = process.env.VALIDATION_MODEL || "gpt-4.1-nano";
...
2. **الهيكل**:
   ...
   - اختم دائماً بقسم "## المصادر الموثوقة" يحتوي على روابط موثوقة.
...
{"is_allowed": boolean, "reason": "short explanation"}`;

function getOpenAIClient() {
  return new OpenAI();
}

async function validateQuery(client: OpenAI, query: string): Promise<{ isValid: boolean; reason?: string }> {
  try {
    const completion = await client.chat.completions.create({
      model: VALIDATION_MODEL,
      messages: [{ role: "user", content: VALIDATION_PROMPT.replace("{{QUERY}}", query) }],
      temperature: 0,
      max_tokens: 150,
    });
    ...
    if (parsed?.is_allowed === true) return { isValid: true };
    return { isValid: false, reason: parsed?.reason || "Not allowed" };
  } catch {
    // If validation fails, allow the query through — the system prompt handles topic restriction
    return { isValid: true };
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = await validateQuery(client, message);
    ...
    if (!rawAnswer) {
      return NextResponse.json({ error: "لم يتم الحصول على إجابة. حاول مرة أخرى." }, { status: 502 });
    }
    ...
    return NextResponse.json({ error: "حدث خطأ أثناء معالجة طلبك. حاول مرة أخرى." }, { status: 500 });
  }
}
